...
We also provide the dataset in a format suited for directly training a neural ranking network.  First, each conversation is split into chunks of consecutive utterances that have at least 2 utterances per user and where the last utterance is generated by the information _provider_. Therefore, if a conversation has 3 turns per user, it will have 2 possible contexts (the first 2 and the first 3 utterances per user). 
...
## Code for running the models used for this research

* [DMN](https://github.com/alexanderblnf/NeuralResponseRankingMTL) - Used for both response ranking and document ranking
* [BERT](https://github.com/alexanderblnf/pytorch-transformers) - Used for all tasks
